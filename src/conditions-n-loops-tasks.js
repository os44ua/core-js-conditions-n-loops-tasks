...
function canQueenCaptureKing(queen, king) {
  const sameFile = queen.x === king.x;
  const sameRank = queen.y === king.y;
  const sameDiag =
...
function isIsoscelesTriangle(a, b, c) {
  ...
  const triangle = positive && a + b > c && a + c > b && b + c > a;
  const iso = a === b || b === c || a === c;
  return triangle && iso;
}
...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < n; i += 1) total += arr[i];

  let left = 0;

  for (let i = 0; i < n; i += 1) {
    const right = total - left - arr[i];
    if (i > 0 && i < n - 1 && left === right) return i;
    left += arr[i];
  }
  return -1;
}
